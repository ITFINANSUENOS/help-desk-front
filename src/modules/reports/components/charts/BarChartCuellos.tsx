    ...
    if (active && payload && payload.length) {
        ...
        return (
            <div className="bg-white p-3 border border-gray-200 rounded shadow-sm text-sm">
                ...
                <p className="text-gray-600">% Atrasos: <span className="font-medium text-gray-900">{Number(data.pct_atrasados).toFixed(1)}%</span></p>
...
export const BarChartCuellos = ({ data }: BarChartCuellosProps) => {
    const chartData = data.map(item => ({
        ...
        duracion_promedio: Number(item.duracion_promedio || 0),
        pct_atrasados: Number(item.pct_atrasados || 0)
    }));

    return (
        <div className="w-full" style={{ height: Math.max(400, data.length * 30) }}>
