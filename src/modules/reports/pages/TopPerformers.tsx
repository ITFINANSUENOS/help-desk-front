...
function PodiumCell({ position, isTop }: { position: number; isTop: boolean }) {
    ...
    if (position === 1) {
        return (
            <div className="flex items-center justify-center">
                <IconMedal size={22} className="text-yellow-400" title="1.º lugar — Oro" />
            </div>
        );
    }
    if (position === 2) {
        return (
            <div className="flex items-center justify-center">
                <IconMedal size={22} className="text-gray-400" title="2.º lugar — Plata" />
            </div>
        );
    }
    if (position === 3) {
        return (
            <div className="flex items-center justify-center">
                <IconMedal size={22} className="text-amber-600" title="3.º lugar — Bronce" />
            </div>
        );
    }
            ...
            {isTop && !isLoading && data && data.length > 0 && (
                <div className="mt-3 flex items-center gap-4 text-xs text-gray-500">
                    <span className="flex items-center gap-1">
                        <IconMedal size={14} className="text-yellow-400" /> 1.º — Oro
                    </span>
                    <span className="flex items-center gap-1">
                        <IconMedal size={14} className="text-gray-400" /> 2.º — Plata
                    </span>
                    <span className="flex items-center gap-1">
                        <IconMedal size={14} className="text-amber-600" /> 3.º — Bronce
                    </span>
                </div>
            )}
        </div>
